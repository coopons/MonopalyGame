...
public class Game {
    ...
    public Game(int totalTurns) {
        ...
        board = new Board();
        player1 = new Player();
        communityDeck = new Deck(DeckType.COMMUNITY);
        chanceDeck = new Deck(DeckType.CHANCE);
        runA = false;
    }
     ...
     */
    public void strategy(StrategyType type) {
        int moves;
        String currSpot;
        runA = false;
        ...
        for (int currTurn = 0; currTurn < totalTurns; currTurn++) {
            ...
            if (board.getJail()) {
                if (type.equals(StrategyType.STRATEGYA)) {
                    jailStrategyA();
                }
                else {
     ...
     */
    public String printBoard(int start, int end) {
        StringBuilder sb = new StringBuilder();
        String result = runA ? "A" : "B";

        sb.append("    Strategy " + result + " Simulation #" + start + " of " + end);
